...
class SQLiteFileRepository(FileRepositoryInterface):
    ...
    def _init_db(self):
        with self._get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_unverified_source ON unverified_files (source)")
    ...
    def update_status(self, source_id: str, status: str):
        with self._get_connection() as conn:
            conn.execute("UPDATE file_records SET status = ? WHERE source_id = ?", (status, source_id))
            conn.commit()
